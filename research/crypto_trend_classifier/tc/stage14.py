...
def main():
    ...
    for g in w.index.levels[0]:
        x = w.loc[g]
        for ref in ('ORIGINAL', 'BUY & HOLD', 'MODEL-FLIP'):
            k = int((x.HYBRID > x[ref]).sum()); n = len(x)
            print(g, 'HYBRID Sharpe >', ref, f'{k}/{n}', 'p=%.2g' % binomtest(k, n).pvalue)
