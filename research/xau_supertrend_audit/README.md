...
## الجزء الثاني: الـ regime كمصنّف للاتجاه (trend classifier)
`trend_eval.py`, `run5.py`, `run6.py` — مقارنة الـ regime (SuperTrend 4h + EMA10 [+ ADX]) مع ملصِّقين مثاليين يرون المستقبل:
1. **Oracle labeling** (Kovačević et al., IEEE Access 2023): برمجة ديناميكية تعظّم العائد مع تكلفة لكل تغيير اتجاه (تتجاهل الارتدادات الصغيرة).
2. **ZigZag / Directional Change / CTL** بعتبة θ = k × التقلب اليومي النموذجي (k = 1..16).

المقاييس: الدقة عند اتخاذ موقف، وقت العكس (wrong-way)، MCC، نسبة الربح المثالي الملتقط، تأخير الدخول/الخروج، نسبة الحركة الضائعة، دقة إشارات "انتهى الاتجاه"، واختبار عشوائي بإزاحة دائرية.
![trend classifier](trend_classifier.png)
